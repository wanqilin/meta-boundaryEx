...
void phydm_basic_dbg_message(void *dm_void)
{
	...
	if (dm->support_ic_type & ODM_RTL8710C){
		if(dm->is_rssi_dump_en == 1 &&  dm->is_linked) {
			printf( "[RTL8710C]:(RSSI %d,IGI 0x%x),CCA{%d,%d},FA{%d,%d},CRCOK{%d,%d,%d},CRCErr{%d,%d,%d},TRXTP{%d,%d},TRXRate{0x%x,0x%x},CFOavg %dkHz,CFOtrk %s%d,0xA08=0x%x,0xAd0=0x%x,0xC4C=0x%x\n",\
			dm->rssi_min,dm->dm_dig_table.cur_ig_value,fa_t->cnt_cck_cca, fa_t->cnt_ofdm_cca, fa_t->cnt_cck_fail, fa_t->cnt_ofdm_fail, fa_t->cnt_cck_crc32_ok, fa_t->cnt_ofdm_crc32_ok, fa_t->cnt_ht_crc32_ok, fa_t->cnt_cck_crc32_error, fa_t->cnt_ofdm_crc32_error, fa_t->cnt_ht_crc32_error, dm->tx_tp, dm->rx_tp,\
			dm->phydm_sta_info[0]->ra_info.curr_tx_rate, dm->rx_rate, cfo_t->CFO_ave_pre, ((cfo_t->crystal_cap > cfo_t->def_x_cap) ? "+" : "-"),DIFF_2(cfo_t->crystal_cap, cfo_t->def_x_cap), odm_get_bb_reg(dm, R_0xa08, MASKDWORD), odm_get_bb_reg(dm, R_0xad0, MASKDWORD), odm_get_bb_reg(dm, R_0xc4c, MASKDWORD));
		}
	}
#endif
	#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
