...
{
#if (RTL8822B_SUPPORT || RTL8197F_SUPPORT || RTL8192F_SUPPORT || RTL8822E_SUPPORT)
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct phydm_dig_struct *dig_t = &dm->dm_dig_table;
	...
	if (dm->support_ic_type & ODM_RTL8822B)
		...
		odm_set_bb_reg(dm, R_0x1928, 0x001c0000, i);
		odm_set_bb_reg(dm, R_0x4028, 0x001c0000, i);
	}
...
void odm_write_dig(void *dm_void, u8 new_igi)
{
	...
	if (dig_t->cur_ig_value != new_igi) {
		...
		else if ((dm->support_ic_type & ODM_RTL8822E) && (dig_t->is_dbg_dyn_big_jump_step1_en ==1))
			phydm_set_big_jump_step(dm, new_igi);
		#endif
...
void phydm_dig_init(void *dm_void)
{
	...
	dig_t->is_dbg_dyn_big_jump_step1_en = false;
	dig_t->fa_source = 0;
...
#if (RTL8822B_SUPPORT || RTL8197F_SUPPORT || RTL8192F_SUPPORT || RTL8822E_SUPPORT)
	if (dm->support_ic_type &
	    (ODM_RTL8822B | ODM_RTL8197F | ODM_RTL8192F | ODM_RTL8822E)) {
		...
		if (dm->support_ic_type & ODM_RTL8822B)
			...
			ret_value = odm_get_bb_reg(dm, R_0x1928, MASKDWORD);

		if (dm->support_ic_type & ODM_RTL8822E){
			dig_t->big_jump_step1 = (u8)((ret_value & 0x001C0000) >> 18);
			dig_t->big_jump_step2 = (u8)((ret_value & 0x03000000) >> 24);
			dig_t->big_jump_step3 = (u8)((ret_value & 0x0C000000) >> 26);
		} else {
		     ...
		     u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "{8} Dyn. step1 big jump:{0:Disable/1:Enable}\n");
	} else {
		PHYDM_SSCANF(input[1], DCMD_DECIMAL, &var1[0]);
		...
		if (var1[0] == 0) {
			...
			dig_t->is_dbg_dyn_big_jump_step1_en = (u8)var1[1];
			PDM_SNPF(out_len, used, output + used, out_len - used,
				 "Dyn. step1 big jump enable = %d\n", dig_t->is_dbg_dyn_big_jump_step1_en);
		}
	}
	*_used = used;
	*_out_len = out_len;
