...
s8 phydm_get_pwdB_jgr3(void *dm_void, boolean is_cck_rate, enum rf_path path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct phydm_physts *physts_table = &dm->dm_physts_table;
	u8 cck_pwdb_path[4] = {1, 16, 20, 24};
	u8 cck_mp_gain_path[4] = {2, 19, 23, 27};
	u8 cck_mp_gain_idx = 0x20;
	s8 val = -127;
	...
	if (is_cck_rate)
		odm_set_bb_reg(dm, R_0x8c0, 0x03C00000, 0x0);
	else
		odm_set_bb_reg(dm, R_0x8c0, 0x03C00000, 0x1);

	value = odm_get_bb_reg(dm, R_0x8c4, MASKDWORD);
	value &= ~0x18050000; /*[28:27],[18],[16]*/
	value |= BIT(18);
	odm_set_bb_reg(dm, R_0x8c4, MASKDWORD, value);

	/*Get mp gain idx*/
	if (is_cck_rate) {
		odm_set_bb_reg(dm, R_0x8c4, 0x3ff, cck_mp_gain_path[path]);
		if (phydm_set_bb_dbg_port(dm, DBGPORT_PRI_3, 0x380)) {
			dbg_port_value = phydm_get_bb_dbg_port_val(dm);
			phydm_release_bb_dbg_port(dm);
			cck_mp_gain_idx = (u8)(dbg_port_value & 0x3f);
		}
	}

	/*Get pwdB*/
	if (is_cck_rate) {
		odm_set_bb_reg(dm, R_0x8c4, 0x3ff, cck_pwdb_path[path]);
	} else {
		odm_set_bb_reg(dm, R_0x8c4, 0x3ff, path + 1);
	}

	if (phydm_set_bb_dbg_port(dm, DBGPORT_PRI_3, 0x380)) {
		dbg_port_value = phydm_get_bb_dbg_port_val(dm);
		phydm_release_bb_dbg_port(dm);
		val = (s8)(dbg_port_value & 0xff);

		if (is_cck_rate) {
			if (cck_mp_gain_idx < physts_table->cck_gi_l_bnd)
				val += ((physts_table->cck_gi_l_bnd -
					cck_mp_gain_idx) << 1);
		}
		val-= 110;
		if (val > 0 || val < -110)
...
s8 phydm_get_pwdB(void *dm_void, boolean is_cck_rate, enum rf_path path)
{
	...
	#ifdef PHYDM_IC_JGR3_SERIES_SUPPORT
	if (dm->support_ic_type & ODM_IC_JGR3_SERIES)
		val = phydm_get_pwdB_jgr3(dm, is_cck_rate, path);
	#endif
