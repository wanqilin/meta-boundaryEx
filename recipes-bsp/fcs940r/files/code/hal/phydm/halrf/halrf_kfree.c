...
void phydm_get_set_pa_bias_offset_8735b(void *dm_void)
{
	...
	u8 pg_pa_bias = 0xff, pg_pa_bias_chk = 0xff;
	s8 pg_pa_bias_2g, pg_pa_bias_5g;

	odm_efuse_one_byte_read(dm, PPG_PABIAS_35B, &pg_pa_bias, false);
	odm_efuse_one_byte_read(dm, PPG_PABIAS_CHECK_35B, &pg_pa_bias_chk, false);

	RF_DBG(dm, DBG_RF_MP, "======>%s hide efuse 0x%x=0x%x 0x%x=0x%x rf->iot_dram_size=%d\n",
		__func__, PPG_PABIAS_35B, pg_pa_bias, PPG_PABIAS_CHECK_35B, pg_pa_bias_chk, rf->iot_dram_size);
	...
	if (pg_pa_bias != 0xff) {
		...
		if (rf->iot_dram_size == HALRF_DRAM_SIZE_128M && (pg_pa_bias_chk & 0xff) == 0xff) {
			pg_pa_bias_2g = pg_pa_bias_2g + 2;
			if (pg_pa_bias_2g > 0x7)
				pg_pa_bias_2g = 0x7;
		}

		if ((pg_pa_bias_chk & 0xff) == 0xfe) {
			if (pg_pa_bias_2g <= -2) {
				pg_pa_bias_2g = pg_pa_bias_2g + 2;
				RF_DBG(dm, DBG_RF_MP, "[kfree] 2G s0 efuse:pa_bias=0x%x <= -2, Real:pg_pa_bias_2g + 2=0x%X\n",
...
void phydm_get_set_dac_dck_8735b(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u8 dac_dck_trim[4] = {0xff, 0xff, 0xff, 0xff};

	odm_efuse_one_byte_read(dm, PPG_DACDCK1_35B, &dac_dck_trim[0], false);
	odm_efuse_one_byte_read(dm, PPG_DACDCK2_35B, &dac_dck_trim[1], false);
	odm_efuse_one_byte_read(dm, PPG_DACDCK3_35B, &dac_dck_trim[2], false);
	odm_efuse_one_byte_read(dm, PPG_DACDCK4_35B, &dac_dck_trim[3], false);

	RF_DBG(dm, DBG_RF_MP, "======>%s hide efuse  0x%x=0x%x\n",
		__func__, PPG_DACDCK1_35B, dac_dck_trim[0]);
	RF_DBG(dm, DBG_RF_MP, "======>%s hide efuse  0x%x=0x%x\n",
		__func__, PPG_DACDCK2_35B, dac_dck_trim[1]);
	RF_DBG(dm, DBG_RF_MP, "======>%s hide efuse  0x%x=0x%x\n",
		__func__, PPG_DACDCK3_35B, dac_dck_trim[2]);
	RF_DBG(dm, DBG_RF_MP, "======>%s hide efuse  0x%x=0x%x\n",
		__func__, PPG_DACDCK4_35B, dac_dck_trim[3]);

	if ((dac_dck_trim[0] & 0xff) == 0xff && (dac_dck_trim[1] & 0xff) == 0xff &&
		(dac_dck_trim[2] & 0xff) == 0xff && (dac_dck_trim[3] & 0xff) == 0xff) {
		RF_DBG(dm, DBG_RF_MP, "[kfree] 8735B DAC DCK Trim no pg\n");
	} else {
		odm_set_bb_reg(dm, 0x18bc, 0x00000004, 0x1);
		odm_set_bb_reg(dm, 0x18bc, 0x00000008, 0x1);
		odm_set_bb_reg(dm, 0x18c8, 0x00000002, 0x1);
		odm_set_bb_reg(dm, 0x18c8, 0x00000004, 0x1);
		odm_set_bb_reg(dm, 0x18c8, 0x00000008, 0x1);
		odm_set_bb_reg(dm, 0x18c8, 0x00000010, 0x1);
		odm_set_bb_reg(dm, 0x18c0, 0x0fc00000, dac_dck_trim[0] & 0x3f);
		odm_set_bb_reg(dm, 0x18c4, 0x0000003f, dac_dck_trim[1] & 0x3f);
		odm_set_bb_reg(dm, 0x18c4, 0x003f0000, dac_dck_trim[2] & 0x3f);
		odm_set_bb_reg(dm, 0x18c4, 0x0fc00000, dac_dck_trim[3] & 0x3f);
		odm_set_bb_reg(dm, 0x1830, 0x40000000, 0x0);
		odm_set_bb_reg(dm, 0x1860, 0x80000000, 0x0);
		odm_set_bb_reg(dm, 0x1860, 0x80000000, 0x1);
		odm_set_bb_reg(dm, 0x1830, 0x40000000, 0x1);

		RF_DBG(dm, DBG_RF_MP, "[kfree] 8735B Do DAC DCK Trim !!!\n");
		RF_DBG(dm, DBG_RF_MP, "[kfree] hide efuse  0x%x[5:0]=0x%x  0x18c0[27:22]=0x%x\n",
			PPG_DACDCK1_35B, dac_dck_trim[0] & 0x3f,
			odm_get_bb_reg(dm, 0x18c0, 0x0fc00000));
		RF_DBG(dm, DBG_RF_MP, "[kfree] hide efuse  0x%x[5:0]=0x%x  0x18c0[5:0]=0x%x\n",
			PPG_DACDCK2_35B, dac_dck_trim[1] & 0x3f,
			odm_get_bb_reg(dm, 0x18c4, 0x0000003f));
		RF_DBG(dm, DBG_RF_MP, "[kfree] hide efuse  0x%x[5:0]=0x%x  0x18c0[21:16]=0x%x\n",
			PPG_DACDCK3_35B, dac_dck_trim[2] & 0x3f,
			odm_get_bb_reg(dm, 0x18c4, 0x003f0000));
		RF_DBG(dm, DBG_RF_MP, "[kfree] hide efuse  0x%x[5:0]=0x%x  0x18c0[27:22]=0x%x\n",
			PPG_DACDCK4_35B, dac_dck_trim[3] & 0x3f,
			odm_get_bb_reg(dm, 0x18c4, 0x0fc00000));
	}
}
...
void phydm_do_new_kfree(void *dm_void)
{
	...
	if (dm->support_ic_type & ODM_RTL8735B) {
		...
		phydm_get_set_dac_dck_8735b(dm);
	}
#endif
