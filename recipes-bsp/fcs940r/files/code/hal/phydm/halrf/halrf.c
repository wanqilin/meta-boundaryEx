				 ...
				 char *output, u32 *_out_len)
{
	...
	if (dm_value[0] == 100) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "12. (( %s ))PA Bias Track\n",
			 ((rf->rf_supportability & HAL_RF_PABIAS_TRACK) ? ("V") :
			 (".")));
			 ...
			 u32 value)
{
	...
	case HALRF_CMNINFO_MP_POWER_TRACKING_TYPE:
		...
		break;
	case HALRF_CMNINFO_DRAM_SIZE:
		rf->iot_dram_size = (u8)value;
		break;
	case HALRF_CMNINFO_DRAM_TYPE:
		rf->iot_dram_type = (u8)value;
		break;
	default:
		break;
	}
...
#if (RTL8721F_SUPPORT == 1)
		case ODM_RTL8721F:
			phy_iq_calibrate_8721f(dm, false);
			break;
#endif
...
void halrf_rfk_self_init(struct dm_struct *dm)
{
	struct _hal_rf_ *rf = &dm->rf_table;
	struct _halrf_rxdck_info *rx_dck = &(rf->halrf_rxdck_info);
...
#if (RTL8730A_SUPPORT == 1)
		case ODM_RTL8730A:
			halrf_txgapk_clear_table_8730a(dm);
		break;
#endif
...
#if (RTL8735B_SUPPORT == 1)
	case ODM_RTL8735B:
		thermal = halrf_get_thermal_8735b(dm, path);
	break;
#endif
...
#if (RTL8735B_SUPPORT == 1)
	if (dm->support_ic_type & ODM_RTL8735B)
		halrf_pabias_track_init_8735b(dm);
#endif
