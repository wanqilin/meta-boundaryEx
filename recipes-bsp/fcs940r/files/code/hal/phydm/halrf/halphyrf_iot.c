...
)
{
	...
	u8 thermal_value = 0, delta, delta_LCK, delta_IQK, delta_pabias, p = 0, i = 0;
	...
	if (cali_info->txpowertrack_control == 4) {
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "TSSI CAL Mode Return!!!\n");
		return;
	}
	...
	delta_pabias = (thermal_value > cali_info->thermal_value_pabias) ? (thermal_value - cali_info->thermal_value_pabias) : (cali_info->thermal_value_pabias - thermal_value);

	/*4 6. If necessary, do LCK.*/
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "(delta, delta_LCK, delta_IQK, delta_PABias) = (%d, %d, %d, %d)\n",
		delta, delta_LCK, delta_IQK, delta_pabias);
	...
	if ((!*dm->is_scan_in_process) && !iqk_info->rfk_forbidden) {
		/*Delta temperature is equal to or larger than 20 centigrade (When threshold is 8).*/
		if (delta_pabias >= c.threshold_pabias) {
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "delta_PABias(%d) >= threshold_pabias(%d)\n", delta_pabias, c.threshold_pabias);

			RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "thermal_value(%d) cali_info->thermal_value_pabias(%d)\n",
				thermal_value, cali_info->thermal_value_pabias);

			if (dm->support_ic_type == ODM_RTL8735B) {
				if ((cali_info->pabias_workaround == false) && (thermal_value < cali_info->thermal_value_pabias)) {
					(*c.pa_bias_track)(dm, true);
					cali_info->pabias_workaround = true;
				}

				if ((cali_info->pabias_workaround == true) && (thermal_value >= cali_info->thermal_value_pabias)) {
					(*c.pa_bias_track)(dm, false);
					cali_info->pabias_workaround = false;
				}
			}

			cali_info->thermal_value_pabias = thermal_value;
		}
	}
